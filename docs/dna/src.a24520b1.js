...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"yZzA":[function(require,module,exports) {
var define;
var global = arguments[3];
/*! Tweakpane 3.0.1 (c) 2016 cocopon, licensed under the MIT license. */
(function (global, factory) {
    ...
    const VERSION = new Semver('3.0.1');
...
exports.Line = exports.Circle = void 0;

// eslint-disable-next-line max-classes-per-file
class Circle extends Path2D {
  /**
   * @param {number} x
   * @param {number} y
   * @param {number} radius
...
exports.Line = Line;
},{}],"bwFq":[function(require,module,exports) {
"use strict";
...
var _geometries = require("../../../utils/geometries");

var _range = require("../../../utils/range");

var _degToRad = require("../../../utils/deg-to-rad");
...
const pane = new _tweakpane.Pane({
  title: 'Parameters',
  expanded: false
});
pane.addInput(settings, 'columns', {
  min: 1,
  max: 50,
  step: 1,
  label: 'length'
});
pane.addInput(settings, 'padding', {
  min: 1,
  max: 100,
  step: 1
});
...
params => {
  ...
  /** @type {Params} */
  {
    ...
    context.rotate((0, _degToRad.degToRad)(10 * time));
    (0, _range.range)(settings.rows).forEach(r => {
      const dy = r - settings.rows / 2;
      (0, _range.range)(settings.columns).forEach(c => {
        const dx = c - settings.columns / 2;
        const r1 = 1 + (1 + Math.sin(time * 2 + c / (settings.columns / 4) - r / (settings.rows / 4))) * settings.radius;
        const r2 = 1 + (1 + Math.sin(time * 2 + c / (settings.columns / 4) - r / (settings.rows / 4) + 2)) * settings.radius;
        const x1 = dx * settings.radius * 2 + dx * settings.padding;
        const y1 = dy * settings.radius * 2 + dy * settings.padding + Math.sin(time * 2 + c / (settings.columns / 6)) * 50;
        const y2 = dy * settings.radius * 2 + dy * settings.padding + Math.sin(time * 2 + c / (settings.columns / 6) + 3) * 50;
        const color = `hsl(${c / settings.columns * 360}, 50%, 50%)`;
        context.fillStyle = color;
        context.strokeStyle = color;
        context.stroke(new _geometries.Line(y1, x1, y2, x1));

        if (r1 < r2) {
          context.fill(new _geometries.Circle(y1, x1, r1));
          context.fill(new _geometries.Circle(y2, x1, r2));
        } else {
          context.fill(new _geometries.Circle(y2, x1, r2));
          context.fill(new _geometries.Circle(y1, x1, r1));
        }
...
},{"tweakpane":"yZzA","canvas-sketch":"bkbf","../../../utils/geometries":"NlR3","../../../utils/range":"bwFq","../../../utils/deg-to-rad":"gAVG"}]},{},["Focm"], null)
//# sourceMappingURL=src.a24520b1.js.map