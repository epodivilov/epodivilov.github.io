...
pane.addInput(settings, 'circles', {
  min: 1,
  max: 50,
  step: 1,
  label: 'length'
});
pane.addInput(settings, 'step', {
  min: 1,
  max: 100,
  step: 1
});
...
params => {
  ...
  /** @type {Params} */
  {
    ...
    (0, _range.range)(settings.circles).forEach(i => {
      ...
      context.beginPath();
      context.arc(0, 0, settings.step * (i + 1), -angle + (t - Math.sqrt(i)), angle + (t - Math.sqrt(i)));
      context.stroke();
    });
  };
}, {
...
},{"tweakpane":"yZzA","canvas-sketch":"bkbf","../../../utils/range":"bwFq"}]},{},["Focm"], null)
//# sourceMappingURL=src.643f8bf8.js.map