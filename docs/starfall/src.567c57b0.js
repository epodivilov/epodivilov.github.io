...
pane.addInput(settings, 'density', {
  min: 1,
  max: 1000,
  step: 1
});
...
f1.addInput(settings.radius, 'min', {
  min: 1,
  step: 1
});
f1.addInput(settings.radius, 'max', {
  min: 1,
  step: 1
});
...
f2.addInput(settings.spikes, 'min', {
  min: 1,
  step: 1,
  max: 9
});
f2.addInput(settings.spikes, 'max', {
  min: 1,
  step: 1,
  max: 9
});
...
function spawnStars(options = {}) {
  return (0, _range.range)(options.density).map(() => {
    const object = new Star({
      x: Math.floor((0, _random.random)(0, options.width)),
      y: Math.floor((0, _random.random)(0, options.height)),
      radius: Math.floor((0, _random.random)(options.radius.min, options.radius.max)),
      spikes: Math.floor((0, _random.random)(options.spikes.min, options.spikes.max)),
      velocity: (0, _random.random)(1, 5)
    });
    return object;
  });
}
...
params => {
  ...
  let circles = spawnStars({ ...settings,
    width,
    height
  });
  pane.on('change', () => {
    circles = spawnStars({ ...settings,
      width,
      height
    });
  });
  ...
  /** @type {Params} */
  {
    ...
    context.rotate((0, _degToRad.degToRad)(135));
    ...
    circles.forEach((circle, i) => {
      ...
      if (circle.x > width) {
        circle.x = (0, _random.random)(-200, -100);
        circle.y = (0, _random.random)(0, height);
      }
...
},{"tweakpane":"yZzA","canvas-sketch":"bkbf","../../../utils/range":"bwFq","../../../utils/deg-to-rad":"gAVG","../../../utils/random":"EyLH"}]},{},["Focm"], null)
//# sourceMappingURL=src.567c57b0.js.map